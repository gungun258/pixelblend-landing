  ...
  try {
    ...
    (function () {
      if (!document.getElementById('pb-to-top-css')) {
        ...
        hideCss.textContent = '#pb-to-top:not(.pb-on){opacity:0!important;visibility:hidden!important;pointer-events:none!important;transform:translateY(18px) scale(0.88)!important}';
      ...
      function pastHero() {
        ...
        if (!grid) return pbScrollY() > Math.max(window.innerHeight * 0.85, 480);
        return grid.getBoundingClientRect().bottom < 56;
      }
      function syncToTop(btn) {
        if (!btn) return;
        if (pastHero()) btn.classList.add('pb-on');
